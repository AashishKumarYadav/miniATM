...
int main()
{
	...
	user1.setData(123456789, "Aryan", 1111, 45000.90, "9876543210");

    // created User ( object )
	atm user2;

    // Set User Details ( into object )         ( Setting Default Data )
	user2.setData(246813579,"Aashish",1234,99999.90,"9897141344");

    // created User ( object )
    atm user3;

    // Set User Details ( into object )         ( Setting Default Data )
    user3.setData(987654321,"Surjeet",9876,83454.91,"1234567890");
	...
	do
	{
		...
		cout << endl << "|<|****Welcome to miniATM****|>|" << endl;
        ...
        // check whether enter values matches with user AccountNo
        if(enterAccountNo == user1.getAccountNo()){
		    user = user1;

	    }// check whether enter values matches with user AccountNo
		else if(enterAccountNo == user2.getAccountNo()){
		    user = user2;
		...
		if ((enterAccountNo == user.getAccountNo()) && (enterPIN == user.getPIN()))
		{
			...
			do
			{
				...
				case 4:								            //  if user presses 4
					...
					user.setMobile(oldMobileNo, newMobileNo);	// now set new mobile no
					break;
